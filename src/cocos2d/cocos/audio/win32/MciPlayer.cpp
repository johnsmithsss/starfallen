...
void MciPlayer::SetMusicVolume(UINT MusicVolume)
{
	if (!_dev)
	{
		return;
	...
	MCI_DGV_SETAUDIO_PARMS mciStatus = { 0 };
	mciStatus.dwItem = MCI_DGV_SETAUDIO_VOLUME;
	mciStatus.dwValue = MusicVolume;
	mciSendCommand(_dev, MCI_SETAUDIO, MCI_DGV_SETAUDIO_ITEM | MCI_DGV_SETAUDIO_VALUE, (DWORD)&mciStatus);
}

UINT MciPlayer::GetMusicVolume() const
{
	if (!_dev)
	{
		return 0;
	...
	mciStatus.dwItem = MCI_DGV_STATUS_VOLUME;
	mciSendCommand(_dev, MCI_STATUS, MCI_STATUS_ITEM, (DWORD)&mciStatus);
